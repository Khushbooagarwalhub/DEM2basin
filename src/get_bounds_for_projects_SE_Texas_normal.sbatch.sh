...
##-----------------------------------------------------------------------------
##
#SBATCH -J dem2basin_bounds_SE_Texas_normal.%j    # Job name
#SBATCH -o dem2basin_bounds_SE_Texas_normal.o%j    # Name of stdout output file (%j expands to jobId)
#SBATCH -e dem2basin_bounds_SE_Texas_normal.e%j    # Name of stderr error file (%j expands to jobId)
#SBATCH -p normal ## <- change this line to 'normal' and then resubmit it         # Queue name
#SBATCH -N 1                  # Total number of nodes requested (48 cores/node)
#SBATCH -n 67                 # Total number of mpi tasks requested
#SBATCH -t 48:00:00    ## <- if above line is 'normal', then this should be '48:00:00'      # Run time (hh:mm:ss) - 2 hours
...
python3 $DEM2BASIN_KAG/src/get_bounds_for_projects_se_normal_Texas.py
